...
- The overall Type I error rate is controlled at level $`\alpha`$
  without Bonferroni correction
- The overall power is the joint probability: Power =
  $`\Pr(\text{Reject } \text{H}_{01} \text{ and Reject } \text{H}_{02})`$
...
``` math
Z_{1} > z_{1-\alpha} \text{ and } Z_{2} > z_{1-\alpha}
```
...
**When to use**: Small/medium samples ($`N < 200`$), extreme
probabilities ($`p < 0.1`$ or $`p > 0.9`$), or when strict Type I error
control is required
...
$`\rho = 0.8`$, approximately 11% reduction in sample size can be
achieved compared to $`\rho = 0`$.
...
3.  **Specify error rates**: Type I error $`\alpha`$ (typically 0.025
    for one-sided) and Type II error $`\beta`$ (typically 0.2 for 80%
    power)
