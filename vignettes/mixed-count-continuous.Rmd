...
2. **Continuous endpoint**: Lung function measure such as ${\rm FEV}_{1}$ (forced expiratory volume in 1 second)
...
- **Mean**: ${\rm E}[X] = \lambda = r \times t$ (rate $\times$ time)
- **Variance**: ${\rm Var}[X] = \lambda + \lambda^{2}/\nu$
- **Dispersion parameter**: $\nu > 0$ controls overdispersion
  - As $\nu \to \infty$: ${\rm NB} \to {\rm Poisson}$ (no overdispersion)
  - Small $\nu$: High overdispersion (variance $\gg$ mean)

**Variance-to-mean ratio (VMR)**:
$${\rm VMR} = \frac{{\rm Var}[X]}{{\rm E}[X]} = 1 + \frac{\lambda}{\nu}$$

When ${\rm VMR} > 1$, data are overdispersed and NB is more appropriate than Poisson.
...
$$X_{i,j,2} \sim {\rm N}(\mu_{j}, \sigma^{2})$$
...
$${\rm H}_{01}: r_{1} \geq r_{2} \text{ vs. } {\rm H}_{11}: r_{1} < r_{2}$$

Equivalently, testing $\beta_{1} = \log(r_{1}) - \log(r_{2}) < 0$.

**For continuous endpoint** (2):
$${\rm H}_{02}: \mu_{1} - \mu_{2} \geq 0 \text{ vs. } {\rm H}_{12}: \mu_{1} - \mu_{2} < 0$$
...
$$Z_{1} = \frac{\hat{\beta}_{1}}{\sqrt{{\rm Var}(\hat{\beta}_{1})}}$$

where:

- $\hat{\beta}_{1} = \log(\bar{X}_{1,1}) - \log(\bar{X}_{2,1})$ is the log rate ratio
- ${\rm Var}(\hat{\beta}_{1}) = \frac{1}{n_{2}}\left[\frac{1}{t}\left(\frac{1}{\lambda_{2}} + \frac{1}{r\lambda_{1}}\right) + \frac{1+r}{\nu r}\right] = \frac{V_{a}}{n_{2}}$
...
Under ${\rm H}_{1}$, the test statistics $(Z_{1}, Z_{2})$ asymptotically follow a bivariate normal distribution (Appendix B in Homma and Yoshida, 2024):
...
- $\mu_{1} = \frac{\sqrt{n_{2}}\beta_{1}}{\sqrt{V_{a}}}$ with $\beta_{1} = \log(r_{1}) - \log(r_{2})$
- $\mu_{2} = \frac{\delta}{\sigma\sqrt{\frac{1+r}{r n_{2}}}}$ with $\delta = \mu_{1} - \mu_{2}$
...
$$1 - \beta = {\rm P}(Z_{1} < z_{\alpha} \cap Z_{2} < z_{\alpha} \mid {\rm H}_{1})$$

Using the bivariate normal CDF $\Phi_{2}$:

$$1 - \beta = \Phi_{2}\left(z_{\alpha} - \frac{\sqrt{n_{2}}(\log r_{1} - \log r_{2})}{\sqrt{V_{a}}}, z_{\alpha} - \frac{\sqrt{n_{2}}(\mu_{1}-\mu_{2})}{\sigma\sqrt{\frac{1+r}{r}}} {\LARGE{\Bigg|}} \gamma\right)$$
