...
- $\omega_{k} = \delta_{k}^{\ast}\sqrt{\frac{r n_{2}}{1 + r}}$ is the non-centrality parameter for endpoint $k$
...
- Higher correlation leads to smaller required sample sizes, except where one endpoint needs so many more patients than the other that it alone fixes the size
...
Note: the unknown variance case integrates over the sampling distribution of the variance estimates by Monte Carlo, so it costs more than the known variance case and, without `set.seed()`, returns a slightly different answer on each call. Increasing `nMC` reduces that variation.

The power depends on the continuous endpoints only through the standardized effects $\delta_{k} / \sigma_{k}$, so multiplying every effect and every standard deviation by a common constant leaves the answer unchanged.
