...
#' @param gamma Dependence parameter for the bivariate binomial distribution (must be NULL if rho is specified)
#'
...
#' dbibinom(N = 100, y1 = 30, y2 = 50, p1 = 0.3, p2 = 0.5, rho = 0.5, gamma = NULL)
#'
...
#' p2 <- 0.5
#' rho <- 0.5
#' sum(outer(0:N, 0:N, function(x, y) dbibinom(N, x, y, p1, p2, rho, gamma = NULL)))
#'
#' # Using gamma parameter directly
#' dbibinom(N = 50, y1 = 20, y2 = 25, p1 = 0.4, p2 = 0.5, rho = NULL, gamma = 0.5)
#'
#' @export
dbibinom <- function(N, y1, y2, p1, p2, rho, gamma = NULL) {

  # Check that only one of rho or gamma is specified
  if (!is.null(rho) & !is.null(gamma)) {
    stop("Either rho or gamma must be specified, but not both")
  }
  ...
  # Define xi (see equation (3) in Homma and Yoshida (2025))
  xi <- p2 + gamma * (p2 - p1)
